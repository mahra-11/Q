"""
1D committor-vs-Q curve, visit-based method, with standard error of the mean
(SEM = std / sqrt(n_visits)) shown as error bars -- reflects confidence in
the estimate itself, rather than the raw spread of individual 0/1 outcomes
(which is inherently large near p=0.5 for any binary variable, regardless
of sample size).
"""
import numpy as np
import pandas as pd
...
OUT_CSV  = "/scratch/mma9420/committor_check/Q/committor_1d_visits_sem.csv"
OUT_PNG  = "/scratch/mma9420/committor_check/Q/committor_1d_visits_sem.png"
N_BINS   = 40
...
ax.errorbar(result["q_bin_center"], result["p_folded_visits"], yerr=result["sem_visits"],
            ...
            markersize=5, alpha=0.9, label="Visit-based (mean +/- SEM)")
...
ax.set_title("Empirical committor vs. Q -- visit-based, with SEM")
ax.legend()
plt.tight_layout()
